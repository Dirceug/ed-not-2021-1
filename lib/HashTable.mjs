...
export class HashTable {
    ...
    constructor(count, hashFunction) {
        ...
        //Criando as truturas finais (DLL) e colocando-as no vetor
        this.#table = []
        for(let i = 0; i < count; i++) this.#table.push(new DoublyLinkedList)

        //Armazenar a função de espalhamento passada para uso posterior
    ...
    remove(val) {
        ...
        //A posição do valor dentro da lista
    ...
    //Retorna verdadeiro se o valor já existir na tabela ou false caso contrário
    includes (val){
        // Determina em QUAL lista PODE estar o valor a ser removido.
        const listPos = this.#hashFunction(val)    
