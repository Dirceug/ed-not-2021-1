...
function insertInList(val){
    ...
    //2º caso: procurando a posição de inserção
        ...
        for (let i = 0; i < list.count; i++){
            ...
            if(valPos > val){
                //Se o valor que já está na lista é MAIOR que o valor que queremos inserir, encontramos a posição de inserção.
                list.insert(i, val)
                return //Sai da função
            }
            
        }
        //se rodamos todo o for sem encontrar a posição de inserção, é porque o novo valor deve ser inserido no final da lista
        list.insertTail(val)
    }
